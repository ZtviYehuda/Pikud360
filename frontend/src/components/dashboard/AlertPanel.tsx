...
export default function AlertPanel({ alerts, isRTL = false }: AlertPanelProps) {
    ...
    return (
      <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 rounded-2xl p-6 flex flex-col items-center justify-center text-center shadow-sm h-64">
        ...
        <h4 className="text-xs font-bold text-slate-850 dark:text-white mb-1">
          {isRTL ? 'הכל תקין' : 'All Clear'}
        </h4>
        <p className="text-slate-450 dark:text-slate-400 text-2xs max-w-xs leading-normal">
          {isRTL ? 'אין התרעות פעילות ברמת הכשירות והמצבה ביחידה זו.' : 'No active alerts or threshold violations for this unit.'}
        </p>
      </div>
    );
  }

  // Get styles, labels and icons based on severity string
  ...
  const getAlertDescription = (alert: AlertData) => {
    ...
    if (isRTL) {
      return `חריגה במדד: ${formattedMetric} (ערך נוכחי: ${alert.current_value} לעומת סף של ${alert.operator} ${alert.threshold})`;
    }
    return `Threshold violation on ${formattedMetric} (Current: ${alert.current_value} vs threshold ${alert.operator} ${alert.threshold})`;
  };
    ...
    <div className="bg-white dark:bg-slate-900 border border-slate-200/60 dark:border-slate-800/80 rounded-2xl p-5 shadow-sm space-y-4">
      <div className="flex items-center justify-between pb-1">
        <h3 className="text-sm font-bold text-slate-800 dark:text-white flex items-center gap-2">
          <span>{isRTL ? 'התרעות כשירות ומצבה' : 'System Alerts'}</span>
      ...
      <div className="space-y-3 max-h-60 overflow-y-auto pr-1">
        {activeAlerts.map((alert, idx) => {
          ...
          return (
            <div
              key={idx}
              className={`p-3 rounded-xl border ${style.bg} ${style.border} flex gap-3 items-start transition-all`}
            >
