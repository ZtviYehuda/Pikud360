...
const TRIGGER_COLORS: Record<string, string> = {
  SICK_EXCEEDED: 'bg-red-500/10 text-red-300 border-red-500/30',
  MANPOWER_SHORTAGE: 'bg-orange-500/10 text-orange-300 border-orange-500/30',
  PENDING_TRANSFER: 'bg-blue-500/10 text-blue-300 border-blue-500/30',
  PENDING_APPROVAL: 'bg-yellow-500/10 text-yellow-300 border-yellow-500/30',
};
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white" dir={isHe ? 'rtl' : 'ltr'}>
      ...
      <div className="p-6 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {loading ? (
          <div className="col-span-3 flex justify-center py-20"><div className="animate-spin rounded-full h-8 w-8 border-2 border-yellow-500 border-t-transparent" /></div>
        ) : rules.length === 0 ? (
          ...
          <div key={rule.id} className={`bg-slate-900/60 border rounded-xl p-5 transition-all ${rule.is_active ? 'border-slate-700/60' : 'border-slate-800/40 opacity-60'}`}>
            <div className="flex items-start justify-between mb-3">
              ...
              <div className="flex gap-1">
                {hasPermission('automation.manage') && (
                  <>
                    <button onClick={() => openEdit(rule)} className="p-1.5 hover:bg-slate-700 rounded-lg text-slate-400 transition-colors"><Edit2 className="h-3.5 w-3.5" /></button>
                    <button onClick={() => handleDeactivate(rule.id)} className="p-1.5 hover:bg-red-900/30 rounded-lg text-red-400 transition-colors"><PowerOff className="h-3.5 w-3.5" /></button>
                  </>
                )}
              </div>
            </div>
            <div className="flex flex-wrap gap-2 mb-3">
              <span className={`px-2 py-0.5 text-xs rounded-full border font-medium ${TRIGGER_COLORS[rule.trigger_event] ?? 'bg-slate-700 text-slate-300 border-slate-600'}`}>
                {rule.trigger_event}
              </span>
              <span className="px-2 py-0.5 text-xs rounded-full border bg-slate-800 text-slate-300 border-slate-700">
                → {rule.action_type}
              </span>
            </div>
            {rule.schedule_cron && (
