    ...
    <div className="space-y-6">
      <div>
        <h1 className="font-heading text-3xl font-bold text-slate-900 dark:text-white">
          {isRTL ? 'מבנה ארגוני' : 'Organizational Structure'}
        </h1>
        <p className="text-slate-500 dark:text-slate-400 mt-1">
          {isRTL ? 'ניהול והצגת ההיררכיה הארגונית והמחלקות' : 'Manage corporate hierarchical charts and department trees.'}
        </p>
      </div>
      ...
      <div className="rounded-xl border border-slate-200/60 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900 glassmorphism">
        ...
        <div className="flex flex-col items-center gap-8 py-8">
          ...
          <div className="rounded-xl border border-brand-200 bg-brand-50/50 p-4 text-center max-w-xs shadow-sm dark:border-brand-900/60 dark:bg-brand-950/20">
            ...
            <h4 className="font-bold text-slate-850 dark:text-white">{isRTL ? 'הנהלה ראשית' : 'Executive Board'}</h4>
            <p className="text-xs text-brand-600 font-semibold">{isRTL ? 'מטה כללי' : 'General HQ'}</p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 w-full max-w-2xl relative">
            ...
            <div className="flex flex-col items-center">
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-center w-full max-w-xs dark:border-slate-800 dark:bg-slate-950">
                ...
                <h5 className="font-bold text-slate-800 dark:text-white">{isRTL ? 'חטיבת תפעול' : 'Operations Division'}</h5>
                <p className="text-xs text-indigo-500 font-semibold">{isRTL ? '2 מחלקות פנימיות' : '2 Departments'}</p>
              </div>
            </div>
            ...
            <div className="flex flex-col items-center">
              <div className="rounded-xl border border-slate-200 bg-slate-50 p-4 text-center w-full max-w-xs dark:border-slate-800 dark:bg-slate-950">
                ...
                <h5 className="font-bold text-slate-800 dark:text-white">{isRTL ? 'חטיבת אבטחה' : 'Security Division'}</h5>
                <p className="text-xs text-purple-500 font-semibold">{isRTL ? 'מחוזות אזוריים' : 'Regional HQ'}</p>
              </div>
            </div>
          </div>
