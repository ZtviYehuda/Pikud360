...
from app.modules.reports.models import ReportRequest, ReportFormat, ReportStatus, ReportType
...
class ReportRepository:
    ...
    def _row_to_request(self, row) -> ReportRequest:
        return ReportRequest(
            ...
            name=str(row[3]),
            report_type=ReportType(str(row[4])),
    ...
    def load_pending_reports(self) -> List[ReportRequest]:
        ...
        try:
            with get_db_connection() as conn:
                with conn.cursor() as cur:
                    cur.execute(query, (ReportStatus.PENDING.value,))
                    return [self._row_to_request(row) for row in cur.fetchall()]
        except Exception:
            return []
