    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg p-6 sm:p-7 rounded-[2rem] border border-rose-200/60 dark:border-rose-900/40 shadow-2xl bg-white dark:bg-slate-950" dir="rtl">
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div className="space-y-0.5">
              ...
              <DialogDescription className="text-xs font-bold text-slate-500 dark:text-slate-400">
                אישור השהיית שוטר והעברתו לארכיון המערכת
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
        ...
        <div className="space-y-4 py-3">
          ...
          <div className="flex items-center gap-2.5 px-4 py-2.5 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-700 dark:text-amber-400 text-xs font-bold">
            <AlertTriangle className="w-4 h-4 shrink-0 text-amber-500" />
            <span>השהיית שוטר אינה מוחקת את נתוניו מהמערכת</span>
          </div>

          {/* Structured Cards List */}
          <div className="space-y-2.5">
            {/* Card 1: Data Preserved */}
            <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200/70 dark:border-slate-800/80 flex items-start gap-3.5 transition-colors">
            ...
            {/* Card 2: Hidden from Commanders */}
            <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200/70 dark:border-slate-800/80 flex items-start gap-3.5 transition-colors">
              <div className="w-8 h-8 rounded-xl bg-rose-500/10 text-rose-600 dark:text-rose-400 flex items-center justify-center shrink-0 mt-0.5 border border-rose-500/20">
            ...
            {/* Card 3: Admin Only Access & Restore */}
            <div className="p-3.5 rounded-2xl bg-slate-50 dark:bg-slate-900/60 border border-slate-200/70 dark:border-slate-800/80 flex items-start gap-3.5 transition-colors">
              <div className="w-8 h-8 rounded-xl bg-purple-500/10 text-purple-600 dark:text-purple-400 flex items-center justify-center shrink-0 mt-0.5 border border-purple-500/20">
        ...
        <DialogFooter className="flex flex-col-reverse sm:flex-row gap-3 pt-2">
          <Button
            ...
            disabled={loading}
            className="rounded-xl font-bold h-11 border-slate-200 dark:border-slate-800 flex-1 hover:bg-slate-100 dark:hover:bg-slate-900"
          >
            ביטול
          </Button>
          <Button
            ...
            onClick={onConfirm}
            disabled={loading}
            className="rounded-xl font-black h-11 bg-rose-600 hover:bg-rose-700 text-white flex-1 shadow-md shadow-rose-600/20 transition-all active:scale-[0.98]"
          >
            {loading ? (
...
export default function EmployeeViewPage() {
  ...
  const handleActiveToggleRequest = () => {
    ...
    if (currentActive) {
      ...
      if (!user?.is_admin) {
        toast.error("רק מנהל מערכת (אדמין) מוסמך להחזיר שוטר לא פעיל למערכת");
        return;
      }
