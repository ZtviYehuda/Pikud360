...
export const DashboardFilters = ({
  ...
  const FilterContent = (
    <div className="flex flex-col h-full bg-card overflow-hidden font-sans">
      ...
      {/* Header */}
      <div className={cn("px-4 sm:px-6 pt-3.5 sm:pt-5 pb-2 flex items-center justify-between shrink-0", isDialogContent && "sm:pl-14")} dir="rtl">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-lg bg-primary/10 text-primary flex items-center justify-center shrink-0">
            <Filter className="w-4 h-4" />
          </div>
          <div>
            <h2 className="text-base sm:text-lg font-bold text-foreground tracking-tight leading-tight">סינון</h2>
            <p className="text-[11px] text-muted-foreground leading-tight hidden sm:block">סינון עובדים לפי יחידה, סטטוס, מעמד וגיל</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          {hasActiveFilters && (
            <button
              ...
              className="inline-flex items-center gap-1.5 text-xs font-semibold text-muted-foreground hover:text-destructive transition-colors cursor-pointer px-2 py-1 rounded-lg hover:bg-destructive/10"
            >
          ...
          {/* Close button on mobile for dialog */}
          {isDialogContent && (
            <button
              type="button"
              onClick={onClose}
      ...
      <div className="px-4 sm:px-6 py-2 shrink-0" dir="rtl">
        <div
          id="filter-tabs"
        ...
        >
          {[
            ...
            <button
              ...
              className={cn(
                "text-[11px] sm:text-xs font-bold py-1.5 rounded-lg transition-all flex items-center justify-center gap-1 cursor-pointer select-none truncate",
                activeTab === tab.id
            ...
            >
              ...
              {tab.count > 0 && (
                <span className="w-4 h-4 rounded-full bg-primary/10 text-primary text-[10px] font-black inline-flex items-center justify-center shrink-0">
                  {tab.count}
                </span>
              )}
            </button>
      ...
      {/* Content Area - Adaptive responsive height */}
      <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-3 sm:py-4 custom-scrollbar max-h-[55dvh] sm:max-h-[380px] min-h-[220px] sm:min-h-[340px]">
        {activeTab === "org" && (
      ...
      {/* Footer */}
      <div className="px-4 sm:px-6 pt-3 pb-6 safe-area-bottom sm:pb-5 shrink-0 bg-card border-t border-border/40" dir="rtl">
        <Button
          id="apply-filters-btn"
          onClick={handleApply}
          className="w-full bg-primary hover:bg-primary/90 text-primary-foreground font-bold rounded-xl h-11 sm:h-12 transition-all active:scale-[0.99] text-xs sm:text-sm shadow-xs cursor-pointer"
        >
          החל סינון
        </Button>
      </div>
